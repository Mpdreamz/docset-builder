...
#pragma warning restore CS0414 // Field is assigned but its value is never used
		...
		public AsciiDocVisitor(TextWriter writer, bool debug = false)
		{
			_debug = debug;
			if (writer == null)
			{
        ...
        public virtual void VisitSource(Source source)
		{
			...
			if (!source.Text.StartsWith(Patterns.Block.Listing))
        ...
        public virtual void VisitUnorderedListItem(UnorderedListItem listItem)
		{
            ...
            var marker = listItem.Marker == UnorderedListMarker.Asterisk ? '*' : '-';
			_writer.Write("{0} ", new string(marker, listItem.Level));
			if (listItem.Count > 1)
			{
			...
			for (int index = 0; index < listItem.Count; index++)
			{
				...
				if (_debug)
					_writer.Write($"[[[{element.GetType().Name}]]]");
        ...
        public virtual void VisitOrderedListItem(OrderedListItem listItem)
		{
			...
			{
				var index = Math.Max(listItem.Number.Value, 0);
				switch (listItem.Numbering)
				{
					case NumberStyle.LowerAlpha:
						_writer.Write($"{Patterns.LowerAlphabet[index]}. ");
						break;
					case NumberStyle.UpperAlpha:
						_writer.Write($"{Patterns.UpperAlphabet[index]}. ");
						break;
        ...
        public virtual void VisitListing(Listing listing)
		{
			...
			if (!listing.Text.StartsWith(Patterns.Block.Listing))
