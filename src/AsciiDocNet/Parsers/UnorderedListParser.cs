...
namespace AsciiDocNet
{
    public class UnorderedListParser : ProcessBufferParserBase
    {
            ...
            ref AttributeList attributes)
        {
            ...
            var marker = UnorderedListMarker.Asterisk;
            if (reader.Line.Trim().StartsWith("-"))
	            marker = UnorderedListMarker.Dash;

            var level = match.Groups["level"].Value;
            var text = match.Groups["text"].Value;
            var listItem = new UnorderedListItem(level.Length) { Marker =  marker };
